...
; XXX coin hustle constants for top speeds...defined in main .asm file
...
PRG008_AB62:
	; Southbird's disassembly had a rare error here; the code is actually loading the 
	; hard-coded value $#18 (24 decimal) and this is in fact where the max walking speed is loaded; the defined 
	; constant is not used for this purpose.
	; LDY #Pad_Input
...
PRG008_ABB8:
	...
	LDA <Temp_Var3	 ; magnitude x velocity
	CMP <Temp_Var14	 ; top x velocity magnitude
...
PRG008_ACEF:
	...
	; TSS - Think if we auto-set for P Speed this should work to always enable flying...
...
; if velocity >= 4 check if we're skidding
	LDA Player_MoveLR
	AND <Pad_Holding
...
PRG008_BC92:
	LDA <Pad_Holding
	AND Pipe_PadDirForEnter,X ; horiz pipe [0,1]?
...
PRG008_BCA7:
	JMP PRG008_CANNOT_ENTER_PIPE	 ; Jump to PRG008_CANNOT_ENTER_PIPE

PRG008_BCAA:
	LDX #$02	 ; X = 2 --> assume down-pipe
	LDA Level_Tile_GndL ; left side ground tile 

	LDY <Player_InAir
	BEQ PRG008_BCC4	 ; If Player is NOT mid air, jump to PRG008_BCC4

	; we're in the air - we can only enter pipe if our head is hitting the ceiling
	...
	JMP PRG008_CANNOT_ENTER_PIPE	 ; Otherwise, jump to PRG008_CANNOT_ENTER_PIPE

PRG008_BCC0:
	INX		 ; X = 3 --> in air, looking for an up pipe instead
...
PRG008_BCD6:
	...
	; Not a pipe 1 or 2 end tile...keep looking
...
PRG008_BCF1:
	SUB <Temp_Var1
	CMP #$02	
	BCS PRG008_CANNOT_ENTER_PIPE	 ; If this pipe tile wasn't in range EITHER, jump to PRG008_CANNOT_ENTER_PIPE
...
PRG008_BCFA:
	...
	LDA <Pad_Holding
	AND Pipe_PadDirForEnter,X ; Enter VERTICAL PIPE (x=2 or 3)
	BEQ PRG008_CANNOT_ENTER_PIPE	 ; If Player is NOT pressing the correct direction to enter pipe, jump to PRG008_CANNOT_ENTER_PIPE

; if we make it here, we are going to enter a VERTICAL PIPE

	LDA Level_PipeMove
	BNE PRG008_CANNOT_ENTER_PIPE	 ; If Player is already in pipe, jump to PRG008_CANNOT_ENTER_PIPE
...
PRG008_BD30:
	...
	CMP #10
					...
					; ? Bail if to close to L edge of pipe??
	BGE PRG008_CANNOT_ENTER_PIPE	 ; If Player_X >= 10 after subtracting 3 (??), jump to PRG008_CANNOT_ENTER_PIPE
...
PipeEnter_XYOffs:
	...
	; Does common stuff to prepare to enter a pipe!  XXX Pipe
PipeEntryPrepare:
...
; XXX X velocity capping here for coin hustle!
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
Player_ApplyXVelocity:
	LDX #$00	; X = 0
...
PRG008_BFAC:
	STA <Temp_Var16		; Store absolute value Player_XVel -> Temp_Var16
	CMP #PLAYER_MAXSPEED
